...
const Homepage = () => {
  ...
  return (
    <div className='min-h-screen relative'>
      ...
      {
        (user?.status === "student" || user?.status === "teacher") &&<b className='text-xl text-[grey] font-semibold'>YOUR CLASSROOM !</b>
      }
      </div>
      {user?.status !== "student" &&
      <div className='flex items-center gap-5 mt-5 justify-center'>
        {(user.status !== "teacher" && !addStudent) &&<button className='py-2 px-4 bg-[teal] text-[white] rounded-md' onClick={()=>{setAddTeacher(true)
